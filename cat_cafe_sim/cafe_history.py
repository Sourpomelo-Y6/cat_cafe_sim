...
class CafeHistoryWindow:
    def __init__(self, parent, session):
        ...
        self.days = self.table(frame, ('日目', '売上', 'うちボーナス', '交流件数', '閉店時所持金', '派遣収入', '子猫費用', '人気', '受入費用', '獲得人気', '目標結果', '席数', '増設費用', '営業区分'))
        ...
        for result in rows:
            ...
            self.days.insert('', 'end', values=(result['day'], f"{summary['revenue']:g}",
                ...
                f"{summary['popularity']:g}" if 'popularity' in summary else '未導入', f"{summary.get('recruitment_expenses',0):g}", f"{summary.get('popularity_gain',0):g}", {'active':'挑戦中','cleared':'クリア','expired':'期限内未達'}.get(summary.get('goal_status'),'未導入'), summary.get('seat_count', 1), f"{summary.get('expansion_expenses',0):g}", '休業日' if result.get('day_type')=='day_off' else '営業日'))
            for cat_id, cat in result['cats'].items():
