...
新規ゲームでは「10日以内に人気150」の目標に挑戦します。好感度につながる反応が得られた接客から閉店時に人気を獲得し、家出による減少後に判定します。「目標・結果…」で達成・未達を確認した後も営業を続けられます。旧セーブでは準備中に任意で開始できます。[人気目標の仕様](Docs/HumanToCatPopularityGoal.md)を参照してください。
...
準備中の「派遣・イベント…」→「保護猫の受け入れ…」で、個性の異なる3匹を確認して迎えられます。初期費用は各200で、支払い後に資金が残る場合だけ実行できます。加入時は休養予定です。[受け入れの初回仕様](Docs/HumanToCatRecruitment.md)を参照してください。

経営ルール未導入の旧セーブ・試遊では、「派遣・イベント…」→「ストレス・家出・経営…」から有効にできます。開始時だけ所持金を最低1,000まで補充し、以降は資金0以下・人気0で終了します。[経営ルールの初回仕様](Docs/HumanToCatManagement.md)を参照してください。

「派遣・イベント…」→「譲渡の設定・申し出…」で、保護猫の譲渡イベントをONにできます（初期OFF）。お客への親しみが80以上でプレイヤーへの好感度を上回ると申し出が発生し、譲渡するか見送るか選びます。[譲渡の仕様](Docs/HumanToCatAdoption.md)を参照してください。
...
通常営業の「派遣・イベント…」から、余った猫を近所のお店へ派遣できます。閉店・休業で期間が進み、帰還結果を確認すると報酬を受け取れます。[派遣・帰還の仕様](Docs/HumanToCatCafeActivities.md)を参照してください。
...
営業前の「出勤・休養…」で担当可能な猫を設定できます。「疲労が高い2匹を休養する案を作成」で予定を提案し、確認・手動変更してから保存できます。全員休養の場合は「今日は休業する」へ案内します。前日の接客量を基準にした出勤時の予測疲労・発症確率と、休養時の予測を比較できます。接客で増える疲労と休養日の回復を記録し、翌日の体力全回復は維持します。[出勤・休養と長期疲労](Docs/HumanToCatCafeShifts.md)を参照してください。
...
「自動割り当て」を選べば、待機順の案内も自動化できます。営業は複数猫・初期2席、増設後3席の同時交流に対応しています。
...
通常営業は初期2席、増設後は3席で同時に交流します。席ごとの状態と会計を確認できます。
...
準備中の「店の増設・設備…」から、暫定500で2席を3席へ増設できます。[増設仕様](Docs/HumanToCatExpansion.md)を参照してください。支払い後に資金が残る場合だけ購入でき、増設後の席数を派遣条件と保存再開にも反映します。

「店の増設・設備…」→「休養設備…」から[休養スペース](Docs/HumanToCatRestEquipment.md)を購入できます。暫定400で、在店休養時の疲労回復＋10（特性補正後に加算）。休養予測・支出履歴・保存再開に対応しています。
