...
| 長期目標・終了条件 | 人気目標1段階・有力者目標とゲームオーバーを実装 | 10日以内に人気150でクリアし、結果確認後も継続可能。資金0以下・人気0で終了。有力者への派遣帰還でも独立してクリア可能。多段階化・6匹案は後続 |
...
| 譲渡済み | 在籍中の割り当て対象から外す | 過去の猫・客との関係や履歴は消さない。6匹判定への算入は未決定 |
...
| 6（特性・加入・人気目標の初回完了） | 特性・猫の加入とクリア目標 | 候補の定期追加・受け入れ・費用・保存を実装済み。人気目標の進捗・達成と継続も初回実装済み。特性3種類の効果と表示・保存も初回実装済み。複数特性・有力者1人の満足度目標も実装済み。多段階化・6匹案は後続。継続的な加入経路は対応済み。6匹案の好感度条件と判定は後続 |
...
人気目標を最初の1段階として実装済み。有力者目標とは独立して達成できる。6匹案の導入は後続であり、すべてを強制同時達成にしない。
