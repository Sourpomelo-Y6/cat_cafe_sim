...
| 長期目標・終了条件 | ゲームオーバーは初回実装／クリアは未決定 | 資金0以下・人気0で終了。店の目標、達成表示、クリア判定は後続 |
...
初回は人気0で終了し、その店は閲覧・保存のみとする。「結果・再開始…」から別の新規ゲームを始められる。クリア案の採用・併存、期限当日の判定は未決定。
...
| 6（保護猫受け入れの初回完了） | 特性・猫の加入とクリア目標 | 固定候補の受け入れ・費用・保存を実装済み。特性の作用点とクリア案の進捗・達成は後続。22匹案を選ぶなら継続的な加入経路が必要 |
