...
派遣1種類・活動状態・帰還確認を初回実装した。[現行仕様](HumanToCatCafeActivities.md)を追加案の時点・条件に優先する。以下の将来案にある開店時出発に対し、初回は準備中の確認で即時出発する。
...
2026-09-21追記：[保護猫の受け入れ](HumanToCatRecruitment.md)を初回実装。準備中に固定3匹の候補から初期費用を払って迎え、休養・出勤・派遣・交流へ接続できる。定期更新や他の加入経路、特性・クリア条件は後続。
...
| 猫の登録・参加・加入 | 保護猫受け入れの初回実装済み | 登録済み猫の起動時参加・テスト猫5匹に加え、準備中に資金を使って固定3匹の候補から迎えられる。候補の定期更新は後続 |
...
| 長期目標・終了条件 | 人気目標1段階とゲームオーバーを実装 | 10日以内に人気150でクリアし、結果確認後も継続可能。資金0以下・人気0で終了。多段階化・他のクリア案は後続 |
...
| 3（出発・帰還・資金報酬完了、中断は後続） | 派遣1種類と報酬 | 条件・期間・出発・帰還・中断・報酬を一巡させる。アイテムを報酬にする前に所持と用途を決める |
...
| 6（保護猫受け入れの初回完了） | 特性・猫の加入とクリア目標 | 固定候補の受け入れ・費用・保存を実装済み。人気目標の進捗・達成と継続も初回実装済み。特性、多段階化・他のクリア案は後続。22匹案を選ぶなら継続的な加入経路が必要 |
| 後続 | 設備・席拡張・顧客と曜日・能力成長 | 上の流れを遊んだ上で追加順を判断。必要な顧客IDや表示などの最小基盤は前段で先に整える |

猫を増やす方法は[加入方法の案と優先度](HumanToCatRecruitmentPlan.md)に整理した。ユーザー指定によりスカウトは実装負担が大きいため低優先度とし、加入や22匹達成の必須経路にしない。[保護猫受け入れ](HumanToCatRecruitment.md)を先行実装し、購入・イベント加入へ共通処理を広げる案。

初回派遣の条件は既存パラメータから始め、未実装の特性を必須にして順序3を止めない。
有力者への派遣は派遣基盤を使うが、満足度とクリア判定は通常の派遣報酬とは別に実装する。
人気目標を最初の1段階として実装済み。他の2案との選択・併存は未決定であり、すべてを強制同時達成にしない。
