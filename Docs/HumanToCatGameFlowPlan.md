...
| 支出・経営の失敗 | 未実装／導入未決定 | 家賃、日次経費、治療費、破産条件はない。現在の売上を経費控除後の利益と呼ばない |
...
期限付きキャンペーンや破産は未決定のまま残し、まず継続できる流れを作る案とする。
...
| 最初の目標 | 到達点の表示と達成後の継続 | 強制終了や失敗条件を導入する必要があるかを分ける |
...
**直近の候補は順序0の仕様化。** 設備を何種類も追加したり、回復量比較を再開したりすることを先行させない。
...
目標達成後も次の準備へ進めること、資金不足でも理由が分かり営業へ戻れることを確認する。
