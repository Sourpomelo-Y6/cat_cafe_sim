...
2026-09-21追記：[プレイヤー交流](HumanToCatPlayerInteraction.md)の初回実装を追加。準備中の交流枠と好感度の保存まで実装済み。譲渡と予防判定・ON/OFFは後続。
...
| 4（交流・好感度保存は完了、譲渡は後続） | プレイヤー交流と譲渡 | 相手別関係、交流枠、引き取りの予防とON/OFFを接続。準備・休業との時間関係を確定する |
...
猫を増やす方法は[加入方法の案と優先度](HumanToCatRecruitmentPlan.md)に整理した。ユーザー指定によりスカウトは実装負担が大きいため低優先度とし、加入や22匹達成の必須経路にしない。加入機能では保護猫受け入れを先行候補とし、購入・イベント加入と共通の処理を使う案。
