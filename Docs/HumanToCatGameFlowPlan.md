...
| 設備の購入・設置・効果 | 休養スペースを初回実装 | 準備中に暫定400で購入し、在店休養時の疲労回復＋10。予測・支出・保存に反映。[仕様](HumanToCatRestEquipment.md)。席ごとの接客設備は後続 |
...
| 6（特性・加入・人気目標の初回完了） | 特性・猫の加入とクリア目標 | 候補の定期追加・受け入れ・費用・保存を実装済み。人気目標の進捗・達成と継続も初回実装済み。特性3種類の効果と表示・保存も初回実装済み。複数特性・有力者1人の満足度目標も実装済み。多段階化・22匹案は後続。継続的な加入経路は対応済み。22匹案の好感度条件と判定は後続 |
...
人気目標を最初の1段階として実装済み。有力者目標とは独立して達成できる。22匹案の導入は後続であり、すべてを強制同時達成にしない。
